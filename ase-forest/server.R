...
shinyServer(function(input, output) {
  ...
  output$table = DT::renderDataTable(prepTable(case_sig), 
  ...
  ## Forest plot to highlight selected cASE
  output$forest = renderPlot({
    
    ## Get selection (row index)
    s = input$table_rows_selected
    
    ## Don't display plot if nothing is selected
    if(length(s)) {
      dat <- ase_filt[ ase_filt$rsID == case_sig$rsID[s] &
                       ase_filt$Individual == case_sig$Individual[s] &
                       ase_filt$Plate == case_sig$Plate[s], ]
      dat <- prepForForest(dat)
      p1 <- ggplot(dat, aes(y = Treatment, x = ASE.Beta, color = treatment))
      p1 +
        geom_vline(xintercept = 0, color = 'red', linetype = 4) +
        geom_point(size = 2) +
        geom_errorbarh(aes(xmin = lb, xmax = ub), size = 1, height = 0.25) +
        scale_color_manual(values = treatmentKey[ unique(sort(dat$treatment)), "DeepColor"]) +
        ylab("") + 
        xlab(expression(hat(beta))) +
        theme_classic() +
        theme(legend.position = "none") +
        ggtitle(paste(case_sig$CellType[s], case_sig$rsID[s], case_sig$Gene[s], sep=' - '))
    }
  })
  ...
  output$iase = renderPlot({
    ...
    if( length(s) ) {
      ...
      ctCol = treatmentKey[treatmentKey$Common_Name==case_sig$Control[s], "DeepColor"]
      txCol = treatmentKey[treatmentKey$Common_Name==case_sig$Treatment[s], "DeepColor"]
      
      p1 <- ggplot(case_sig[s,])
      p1 +
